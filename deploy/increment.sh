...
# File:
# wpmktgengine-lifter-lms.php

# Get needed versions from a
PLUGIN_CURRENT_VERSION=$(awk '/   Version/{print $NF}' wpmktgengine-lifter-lms.php)
...
if [[ $response = "yes" ]] || [[ $response = "y" ]] || [[ -z $response ]]; then
  ...
  if [ "$(uname)" == "Darwin" ]; then
    # macOs
    sed -i "" "s/${PLUGIN_CURRENT_VERSION}/${PLUGIN_NEXT_VERSION}/g" wpmktgengine-lifter-lms.php
